...
class ReportDateStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $earlyAllowance = ReportClass::where('month', '10-2025')
            ->where('created_at', '<', Carbon::create(2025, 11, 1))
            ->sum('allowance');

        $lateAllowance = ReportClass::where('month', '10-2025')
            ->where('created_at', '>=', Carbon::create(2025, 11, 1))
            ->sum('allowance');

            //total allowance current month where status is paid
            $paidAllowance = ReportClass::where('month', '10-2025')
            ->where('allowance_note','like','dah_bayar')
            ->sum('allowance');
           
            //total allowance balance where status not paid
            $unpaidAllowance = ReportClass::where('month', '10-2025')
            ->where('allowance_note','not like', 'dah_bayar')
            ->sum('allowance');
     

        return [
            Stat::make('Jumlah Elaun Hantar Sebelum 1/11/25', 'RM' . number_format($earlyAllowance, 2))
                ->color('success')
                ->extraAttributes([
